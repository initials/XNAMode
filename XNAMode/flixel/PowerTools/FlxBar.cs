...
namespace org.flixel
{
    ...
    public class FlxBar : FlxGroup
    {
        ...
        public  const uint FILL_LEFT_TO_RIGHT = 1;
        public  const uint FILL_RIGHT_TO_LEFT = 2;
        public  const uint FILL_TOP_TO_BOTTOM = 3;
        public  const uint FILL_BOTTOM_TO_TOP = 4;
        public  const uint FILL_HORIZONTAL_INSIDE_OUT = 5;
        public  const uint FILL_HORIZONTAL_OUTSIDE_IN = 6;
        public  const uint FILL_VERTICAL_INSIDE_OUT = 7;
        public  const uint FILL_VERTICAL_OUTSIDE_IN = 8;

        private  const uint BAR_FILLED = 1;
        ...
        public void setRange(float min, float max)
        {
            if (max <= min)
            {
                Console.WriteLine("FlxBar: max cannot be less than or equal to min");
                return;
            }
        ...
        public float percent
        {
	        get
	        {
                ...
                return (float)Math.Floor((value / range) * 100);
	        }
	        set
	        {
            if (percent >= 0 && percent <= 100)
            {
                updateValue(pct * percent);
