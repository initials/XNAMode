...
NSString * ImgPlayer = @"chars_50x80.png";
...
- (void) update
{
//    unsigned int p = ((rand()&255)<<16)+((rand()&255)<<8)+(rand()&255);
//    [self setColor:p];
...
//        NSLog(@"%@ %f %f, %d", _curAnim.name , self.velocity.y, self.y, onFloor);
    ...
    if (self.isPlayerControlled ) {
        ...
        if (self.isMale && !self.dead) {
            ...
            } else if (self.scale.x < 0 && ((FlxG.touches.virtualPadButton1 && FlxG.touches.newTouch) || FlxG.touches.iCadeBBegan)&& !self.dead && self.dontDash) { //FlxG.touches.swipedDown  ||
                self.velocity = CGPointMake(-600, self.velocity.y);
                airDashTimer=0;
                self.isAirDashing=YES;
                //[FlxG playWithParam1:@"whoosh.caf" param2:0.1 param3:NO];
